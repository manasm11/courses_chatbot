...
data = yaml.full_load(open("courses_final4.yaml"))
courses = []
attributes = []
...
class Dialogflow:
    def __init__(self, project_id="bot-test-303915", session_id=123456789):
        ...
        self.__courses = []
        self.__attributes = []
        self.__data = yaml.full_load(open("courses_final4.yaml"))
        ...
        for course in self.__courses:
            if self.__attributes != []:
                for attribute in self.__attributes:
                    ...
                    r = (
                        response_from_dialogflow.replace("__ic__", course_data["ic"])
                        .replace("__units__", str(course_data["units"]))
                        .replace("__code__", course_data["code"])
                        .replace("__textbook__", course_data["textbook"])
                        .replace("__compre__", course_data["compre"])
                        .replace("__course__", course)
                        .replace("__attribute__", attribute)
                        .replace("__attribute_value__", str(attribute_data))
                    )
